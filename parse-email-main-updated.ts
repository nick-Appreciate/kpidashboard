...
Deno.serve(async (req) => {
  try {
    ...
    // Check for renewal summary by filename or type override
    const isRenewalSummary = actualFilename.includes("renewal") || 
                           actualFilename.includes("renewal_summary") ||
                           typeOverride === "renewal";
    
    console.log(`Processing file: ${filename}, type override: ${typeOverride}, isRenewalSummary: ${isRenewalSummary}`);
    ...
    if (isRenewalSummary) {
      ...
      const result = { table: "renewal_summary", inserted: data?.length || 0, total: records.length };
      return new Response(JSON.stringify({ success: true, ...result }), {
        headers: { "Content-Type": "application/json" },
      });
    }

    // For non-renewal files, forward to the original parse-email function
    // or return an error if this is meant to be renewal-only
    return new Response(
      JSON.stringify({
        error: "This is a simplified renewal parser. Use the main parse-email function for other file types.",
      }),
