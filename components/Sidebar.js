...
export default function Sidebar({ user, onLogout }) {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="h-16 flex items-center justify-center border-b border-[var(--glass-border)] p-2">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-[var(--glass-border)]">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 bg-accent/20 rounded-full flex items-center justify-center flex-shrink-0" style={{ width: '2rem', height: '2rem', minWidth: '2rem' }}>
            <span className="text-accent text-sm font-medium">
              {user?.name?.charAt(0) || user?.email?.charAt(0)?.toUpperCase() || '?'}
            </span>
          </div>
          <div className={`transition-opacity duration-200 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
            <p className="text-sm text-slate-200 font-medium whitespace-nowrap">{user?.name || 'User'}</p>
            <p className="text-xs text-slate-500 whitespace-nowrap">{user?.email}</p>
          </div>
        </div>
        <button
          onClick={(e) => {
          ...
          className={`w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-500 hover:text-slate-200 hover:bg-white/5 rounded-lg transition-all duration-200 ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        >
