...
import type { DuplicateGroup } from "../../types/bookkeeping";

interface DuplicatesTabProps {
  userEmail: string | undefined;
}

export default function DuplicatesTab({ userEmail }: DuplicatesTabProps) {
  ...
  const handleResolve = async (groupId: number, note: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setGroups(prev => prev.map(g => g.id === groupId ? { ...g, resolved: true, resolved_by: userEmail || null, resolved_at: new Date().toISOString(), resolved_note: note } : g));
  ...
  const handleUnresolve = async (groupId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        setGroups(prev => prev.map(g => g.id === groupId ? { ...g, resolved: false, resolved_by: null, resolved_at: null, resolved_note: null } : g));
        setUnresolvedCount(prev => prev + 1);
      }
    } catch (err) {
  ...
  const formatAmount = (amt: number) => '$' + amt.toLocaleString('en-US', { minimumFractionDigits: 2 });

  const billLink = (billId: string) => `https://appreciateinc.appfolio.com/accounting/payable_invoices/${billId}`;
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="flex gap-2 text-xs">
            <span className="px-2 py-1 bg-amber-500/15 text-amber-400 rounded">{unresolvedCount} unresolved</span>
            <span className="px-2 py-1 bg-slate-500/15 text-slate-400 rounded">{totalCount} total</span>
          </div>
        </div>
          ...
          <div key={month} className="mb-6">
            ...
            <div className="space-y-2">
              {monthGroups.map(group => {
                ...
                return (
                  <div key={group.id} className={`border rounded-lg transition-colors ${isResolved ? 'border-[var(--glass-border)] bg-white/[0.02] opacity-60' : 'border-[var(--glass-border)] bg-surface-overlay/50 hover:border-[var(--glass-border-hover)]'}`}>
                    <button onClick={() => toggleExpand(group.id)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {isResolved ? (
                          <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 bg-emerald-500/15 text-emerald-400 rounded"><CheckCircle2 className="w-3 h-3" />Resolved</span>
                        ) : (
                          <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 bg-amber-500/15 text-amber-400 rounded"><AlertTriangle className="w-3 h-3" />{group.dup_count} entries</span>
                        )}
                      </div>
                    </button>
                          ...
                          <tbody>
                            {group.bills.map((bill, i) => (
                              <tr key={`${bill.bill_id}-${i}`} className="border-b border-white/[0.03] last:border-0">
                                <td className="py-2 pr-3"><a href={billLink(bill.bill_id)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-accent hover:text-accent-light">{bill.bill_id}<ExternalLink className="w-3 h-3" /></a></td>
                                <td className="py-2 pr-3 text-slate-300">{bill.bill_date || '—'}</td>
                                <td className="py-2 pr-3 text-slate-400">{bill.bill_number || '—'}</td>
                                <td className="py-2 pr-3"><span className={`px-1.5 py-0.5 rounded text-[10px] ${bill.status === 'Paid' ? 'bg-emerald-500/15 text-emerald-400' : bill.status === 'Unpaid' ? 'bg-amber-500/15 text-amber-400' : 'bg-slate-500/15 text-slate-400'}`}>{bill.status || '—'}</span></td>
                        ...
                        <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.03]">
                          {isResolved ? (
                            <div className="flex items-center justify-between w-full">
                              ...
                              <button onClick={() => handleUnresolve(group.id)} disabled={actionId === group.id} className="flex items-center gap-1 text-xs px-2 py-1 bg-white/5 text-slate-400 rounded hover:bg-white/10 disabled:opacity-50">
                                <Undo2 className="w-3 h-3" />{actionId === group.id ? '...' : 'Unresolve'}
                              </button>
                            </div>
                          ) : (
                            <div className="flex items-center gap-2 ml-auto">
                              <button onClick={() => setResolveModal({ group, note: '' })} className="flex items-center gap-1 text-xs px-3 py-1.5 bg-emerald-500/15 text-emerald-400 rounded-lg hover:bg-emerald-500/25 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="glass-card max-w-md w-full p-6 mx-4">
            ...
            <div className="flex gap-3 mt-4 justify-end">
              <button onClick={() => setResolveModal(null)} className="px-3 py-1.5 text-xs text-slate-400 hover:text-slate-200 transition-colors">Cancel</button>
              <button
              ...
              >
                {actionId === resolveModal.group.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
